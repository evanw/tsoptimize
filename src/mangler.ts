...
function replaceChildren(node: ts.Node, cb: (node: ts.Node) => ts.Node): void {
  switch (node.kind) {
...
function toBoolean(node: ts.Expression): boolean {
  switch (node.kind) {
    case SyntaxKind.FalseKeyword:
    case SyntaxKind.NullKeyword: {
      return false;
...
function toNumber(node: ts.Expression): number {
  switch (node.kind) {
    case SyntaxKind.FalseKeyword:
    case SyntaxKind.NullKeyword: {
      return 0;
...
function toString(node: ts.Expression): string {
  switch (node.kind) {
    case SyntaxKind.FalseKeyword: {
      return 'false';
    }
...
function createBoolean(value: boolean, pos?: number, end?: number): ts.Expression {
  return ts.createNode(value ? SyntaxKind.TrueKeyword : SyntaxKind.FalseKeyword, pos, end) as ts.Expression;
}

function createNumber(value: number, pos?: number, end?: number): NumericLiteral {
  let node = ts.createNode(SyntaxKind.NumericLiteral, pos, end) as NumericLiteral;
  node.value = value;
  return node;
}

function createString(value: string, pos?: number, end?: number): ts.StringLiteral {
      ...
      case SyntaxKind.PrefixUnaryExpression: {
        ...
        if (isLiteral(operand)) {
          switch ((node as ts.PrefixUnaryExpression).operator) {
            case SyntaxKind.ExclamationToken: return createBoolean(!toBoolean(operand), node.pos, node.end);
            case SyntaxKind.MinusToken: return createNumber(-toNumber(operand), node.pos, node.end);
            case SyntaxKind.PlusToken: return createNumber(toNumber(operand), node.pos, node.end);
            case SyntaxKind.TildeToken: return createNumber(~toNumber(operand), node.pos, node.end);
          }
        }
        break;
      }
          ...
          switch ((node as ts.BinaryExpression).operatorToken.kind) {
            case SyntaxKind.PlusToken: {
              if (left.kind == SyntaxKind.StringLiteral || right.kind == SyntaxKind.StringLiteral) {
                return createString(toString(left) + toString(right), node.pos, node.end);
              }
              return createNumber(toNumber(left) + toNumber(right), node.pos, node.end);
            }

            case SyntaxKind.MinusToken: return createNumber(toNumber(left) - toNumber(right), node.pos, node.end);
            case SyntaxKind.AsteriskToken: return createNumber(toNumber(left) * toNumber(right), node.pos, node.end);
            case SyntaxKind.SlashToken: return createNumber(toNumber(left) / toNumber(right), node.pos, node.end);
            case SyntaxKind.PercentToken: return createNumber(toNumber(left) % toNumber(right), node.pos, node.end);
            case SyntaxKind.AsteriskAsteriskToken: return createNumber(toNumber(left) ** toNumber(right), node.pos, node.end);

            case SyntaxKind.AmpersandToken: return createNumber(toNumber(left) & toNumber(right), node.pos, node.end);
            case SyntaxKind.BarToken: return createNumber(toNumber(left) | toNumber(right), node.pos, node.end);
            case SyntaxKind.CaretToken: return createNumber(toNumber(left) ^ toNumber(right), node.pos, node.end);

            case SyntaxKind.LessThanLessThanToken: return createNumber(toNumber(left) << toNumber(right), node.pos, node.end);
            case SyntaxKind.GreaterThanGreaterThanToken: return createNumber(toNumber(left) >> toNumber(right), node.pos, node.end);
            case SyntaxKind.GreaterThanGreaterThanGreaterThanToken: return createNumber(toNumber(left) >>> toNumber(right), node.pos, node.end);
          }
        }
        break;
      }
